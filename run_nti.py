...
# ########## edit ##########
# prompts = ["meat balls on white plate",
#            "meat sushi on white plate"
#         ]
# cross_replace_steps = {'default_': .4,}
# self_replace_steps = .6
# blend_word = None
# blend_word = ((('balls',), ("sushi",))) # for local edit. If it is not local yet - use only the source object: blend_word = ((('cat',), ("cat",))).
# eq_params = {"words": ("sushi",), "values": (2,)} # amplify attention to the word "tiger" by *2 
...
# ########## edit ##########
# prompts = ["orange van with surfboards on top",
#            "orange van with flowers on top"
#         ]
# cross_replace_steps = {'default_': .4,}
# self_replace_steps = .6
# blend_word = None
# blend_word = ((('surfboards',), ("flowers",))) # for local edit. If it is not local yet - use only the source object: blend_word = ((('cat',), ("cat",))).
# eq_params = {"words": ("flowers",), "values": (2,)} # amplify attention to the word "tiger" by *2 
# controller = make_controller(prompts, False, cross_replace_steps, self_replace_steps, blend_word, eq_params)
# images, _ = run_and_display(prompts, controller, run_baseline=False, latent=x_t, uncond_embeddings=uncond_embeddings,)
# Image.fromarray(np.concatenate(images, axis=1)).save(f"{output_dir}/{PREFIX}-1.png")

# ########## edit ##########
# prompts = ["A cat sitting on a wooden chair",
...
# eq_params = {"words": ("dog",), "values": (2,)} # amplify attention to the word "tiger" by *2 
# controller = make_controller(prompts, False, cross_replace_steps, self_replace_steps, blend_word, eq_params)
# images, _ = run_and_display(prompts, controller, run_baseline=False, latent=x_t, uncond_embeddings=uncond_embeddings,
#                             **inference_kwargs)
# Image.fromarray(np.concatenate(images, axis=1)).save(f"{output_dir}/{PREFIX}-4.png")

# ########## edit ##########
# prompts = ["A cat sitting on a wooden chair",
#            "A tiger sitting on a wooden chair"
#         ]
# cross_replace_steps = {'default_': .4,}
# self_replace_steps = .6
# blend_word = None
...
########## edit ##########
# prompts = ["white tiger on brown ground",
...
# self_replace_steps = .6
# blend_word = None
# blend_word = ((('tiger',), ("cat",))) # for local edit. If it is not local yet - use only the source object: blend_word = ((('cat',), ("cat",))).
# eq_params = {"words": ("cat",), "values": (2,)} # amplify attention to the word "tiger" by *2 
...
# ########## edit ##########
# prompts = ["a plate with steak on it",
#            "a plate with pizza on it"
#         ]
# cross_replace_steps = {'default_': .4,}
# self_replace_steps = .6
# blend_word = None
# blend_word = ((('steak',), ("pizza",))) # for local edit. If it is not local yet - use only the source object: blend_word = ((('cat',), ("cat",))).
# eq_params = {"words": ("pizza",), "values": (2,)} # amplify attention to the word "tiger" by *2 
# controller = make_controller(prompts, False, cross_replace_steps, self_replace_steps, blend_word, eq_params)
# images, _ = run_and_display(prompts, controller, run_baseline=False, latent=x_t, uncond_embeddings=uncond_embeddings,
#                             **inference_kwargs)
# Image.fromarray(np.concatenate(images, axis=1)).save(f"{output_dir}/{PREFIX}-1.png")
